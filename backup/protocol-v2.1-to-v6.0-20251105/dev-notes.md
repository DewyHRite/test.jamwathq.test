...
### 2025-11-04 Protocol Update - Mandatory Backup & Rollback (Gojo)

**Session**: Protocol Enhancement Session
**Invoked**: Gojo (Mission Control)
**Protocol Version**: 2.0 (bumped from 1.0)

**Changes Implemented**:

1. **Mandatory Backup Protocol**:
   - ALL implementations now REQUIRE backup creation BEFORE code changes
   - Enforced in Yuuji and Megumi checklists
   - No exceptions - backup first, code second
   - Naming convention: `backup/[feature-name]-YYYYMMDD/`
   - BACKUP_MANIFEST.md required for all backups

2. **Comprehensive Rollback Procedures**:
   - Three rollback options documented:
     - File-level rollback (preferred - surgical fixes)
     - Git-level rollback (for committed changes)
     - Directory-level rollback (nuclear option)
   - Rollback verification checklist added
   - Rollback report template created

3. **Files Created/Modified**:
   - `domain_zero/CLAUDE.md` - Canonical protocol (22KB) with full Section 4: Backup & Rollback
   - `CLAUDE.md` - Updated index pointing to domain_zero as canonical source
   - `docs/CLAUDE.md` - Reference copy of protocol
   - `docs/BACKUP_ROLLBACK_PROTOCOL.md` - Quick reference guide (9KB)
   - `docs/project-state.json` - Updated protocol version to 6.0

4. **Key Features**:
   - Backup retention policy (30/60 days, permanent for critical)
   - Automated cleanup scripts
   - Backup testing/drill procedures
   - Integration with Yuuji/Megumi workflows
   - Emergency rollback procedures

**Rationale**:
- Prevent data loss from failed implementations
- Enable rapid recovery from broken changes
- Establish safety net for experimental features
- Enforce discipline in development workflow

**Protocol Location Hierarchy**:
```
domain_zero/CLAUDE.md        ← CANONICAL SOURCE (22KB)
├── CLAUDE.md                ← Index (points to domain_zero)
└── docs/
    ├── CLAUDE.md            ← Reference copy
    └── BACKUP_ROLLBACK_PROTOCOL.md  ← Quick reference
```

**Impact on Workflows**:
- **Yuuji**: Must create backup before ANY implementation (mandatory)
- **Megumi**: NEVER creates backups because Megumi NEVER modifies code
- **Role Separation**: Megumi = READ-ONLY audits, Yuuji = ALL code modifications

**Status**: Protocol v2.0 active, project-state.json updated to protocol v6.0

---

# Moved: dev-notes

This file has been relocated to keep the docs organized:

→ New location: `docs/notes/dev-notes.md`

Please update any references to use the path above.
