...
console.log('🔧 Updating gear icon styling across ALL remaining pages...\n');
...
// HTML variations to search for and replace
const oldHTMLVariations = [
  ...
  `    <a href="#footer" class="floating-gear-icon" aria-label="Contact Support">
      <i class="fa fa-cog"></i>
    </a>`
];
...
files.forEach(file => {
  ...
  try {
    ...
    if (content.includes(oldCSS)) {
      ...
      console.log(`⚠️  CSS pattern not found in ${file} - may have different formatting`);
    }

    // Replace HTML - try all variations
    let htmlReplaced = false;
    for (const oldHTML of oldHTMLVariations) {
    ...
    if (!htmlReplaced) {
      console.log(`⚠️  HTML pattern not found in ${file} - may have different formatting`);
    }
...
console.log('\nNote: Files showing warnings may need manual updates due to different formatting.');
