// agencies-page.js - Main page logic for agencies.html
// Extracted from inline script for CSP compliance
// See docs/CSP_INLINE_STYLE_FIX_20251031.md
...
// Store pending review data for TOS confirmation - use window scope for cross-file access
...
function openJamaicaLegalModal(event) {
  ...
  if (!modal) {
    console.error('Jamaica Legal modal element not found in DOM');
    return;
  }
  modal.style.display = "block";
    ...
    try {
      ...
      if (data.success && data.reviews && data.reviews.length > 0) {
        ...
        const reviewsHTML = data.reviews.map(review => {
          ...
          const safeFirstName = escapeHTML(review.userFirstName);
          const safeComments = escapeHTML(review.comments);
          const safeProfilePic = escapeHTML(review.userProfilePicture);
          ...
          const avatar = review.userProfilePicture
            ? `<img src="${safeProfilePic}" alt="${safeFirstName}" style="width: 40px; height: 40px; border-radius: 50%; margin-right: 10px; object-fit: cover;" />`
            : `<div style="width: 40px; height: 40px; border-radius: 50%; background: #ffee00; color: #000; display: flex; align-items: center; justify-content: center; margin-right: 10px; font-weight: bold; font-size: 18px;">${safeFirstName.charAt(0).toUpperCase()}</div>`;
            ...
            <div class="review-item" style="background: #1a1a1a; padding: 1em; margin-bottom: 1em; border-radius: 6px; border-left: 3px solid #ffee00; word-wrap: break-word; overflow-wrap: break-word; word-break: break-word;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5em;">
                <div style="display: flex; align-items: center;">
                  ${avatar}
                  <strong style="color: #ffee00; word-wrap: break-word; overflow-wrap: break-word;">${safeFirstName}</strong>
                </div>
              ...
              <p style="color: #fff; line-height: 1.6; word-wrap: break-word; overflow-wrap: break-word; word-break: break-word;">${safeComments}</p>
            </div>
          `;
        }).join('');
...
function openTOSModal() {
  ...
  if (!modal || !checkbox) {
    console.error('TOS modal elements not found in DOM');
    return;
  }
  checkbox.checked = false;
  toggleTOSAcceptButton();

  // Add event listener for checkbox (CSP-compliant - removes inline onchange)
  ...
  setTimeout(() => {
    const cb = document.getElementById("tosCheckbox");
    if (cb) cb.focus();
  }, 100);
}
...
function toggleTOSAcceptButton() {
  ...
  if (!checkbox || !acceptBtn) {
    console.error('TOS checkbox or accept button not found in DOM');
    return;
  }
...
async function acceptTOS() {
  ...
  // Check if this is a modal submission (new system)
  if (window.pendingReviewData.isModalSubmission) {
    // Modal submission - use the formData directly
    const { agencyId, formData } = window.pendingReviewData;
    ...
    try {
      ...
      if (result && result.success) {
        ...
        // Close the review modal if it's still open
        if (window.closeReviewModal && typeof window.closeReviewModal === 'function') {
  ...
  // Inline form submission (old system for backward compatibility)
  const { agencyId, form } = window.pendingReviewData;
...
// CRIT-003 FIX: validateAndSubmitReview function moved to agencies.js
// Event delegation now handles all 70 form submissions without inline handlers
// See scripts/agencies.js lines 428-486 for implementation
...
document.addEventListener('keydown', function(event) {
  if (event.key === 'Escape' || event.keyCode === 27) {
    ...
    if (loginModal && loginModal.style.display === 'block') {
      closeLoginModal();
      window.pendingReviewData = null; // Clear pending data when user cancels
    }
  }
});
 ...
 * Now with rate limiting protection (100ms delay between requests)
 */
async function loadAllAgencyRatings() {
  ...
  for (const wrapper of agencyWrappers) {
    ...
    // Add 100ms delay between requests to prevent rate limiting
    await delay(100);
  }
