...
(function() {
	 ...
	 * HIGH-010 FIX: Replaced innerHTML with safe DOM methods to prevent XSS
	 */
		...
		containers.forEach(container => {
			...
			if (count === 0) {
				if (!noResultsMsg) {
					...
					// Create icon (XSS-safe: no innerHTML)
					const icon = document.createElement('i');
					icon.className = 'fas fa-search';
					icon.style.cssText = 'font-size: 2em; margin-bottom: 0.5em; display: block; color: #ffee00;';

					// Create main message (XSS-safe: textContent auto-escapes)
					const mainMsg = document.createElement('p');
					mainMsg.textContent = 'No states found matching your search.';

					// Create hint message (XSS-safe: textContent auto-escapes)
					const hintMsg = document.createElement('p');
					hintMsg.style.cssText = 'font-size: 0.9em; margin-top: 0.5em;';
					hintMsg.textContent = 'Try a different search term.';

					// Assemble DOM tree safely
					noResultsMsg.appendChild(icon);
					noResultsMsg.appendChild(mainMsg);
					noResultsMsg.appendChild(hintMsg);
